"""
==========================================================================
APPLE INC. (AAPL) — COMPLETE DCF VALUATION MODEL (FIXED)
==========================================================================
Fixed: NaN handling in historical metrics & auto-tune
==========================================================================
"""
...
# ========================================================================
# SECTION 1: CONFIGURATION & FALLBACK DATA
# ========================================================================

PEER_TICKERS = ["MSFT", "GOOGL", "AMZN", "META", "NVDA", "AVGO", "CRM"]

FALLBACK_FINANCIALS = {
    "revenue":           391_035_000_000,
    "cost_of_revenue":   214_137_000_000,
    "gross_profit":      176_898_000_000,
...
class DCFAssumptions:
    ...
    beta:                 float = 1.24
    cost_of_debt_pretax:  float = 0.04
    debt_weight:          float = 0.25
    terminal_growth_rate: float = 0.025
    bull_revenue_bump:  float =  0.02
    bear_revenue_cut:   float = -0.02
    bull_margin_bump:   float =  0.03
    bear_margin_cut:    float = -0.03
    bull_wacc_shift:    float = -0.01
    bear_wacc_shift:    float =  0.015
    mc_revenue_growth_mean:  float = 0.06
    mc_revenue_growth_std:   float = 0.03
    mc_ebitda_margin_mean:   float = 0.34
    mc_ebitda_margin_std:    float = 0.04
    mc_wacc_mean:            float = 0.10
    mc_wacc_std:             float = 0.015
    mc_terminal_growth_mean: float = 0.025
...
class DataHandler:
    ...
    def get_historical_metrics(self, years: int = 4) -> Dict:
        """Compute historical averages — FIXED: filters NaN/None properly"""
        metrics = {
        ...
        # Revenue growth — only use valid numeric values
        revenues = []
        for c in cols:
            rev = None
        ...
        for i in range(len(revenues) - 1):
            ...
            if r1 and r2 and r2 > 0 and np.isfinite(r1) and np.isfinite(r2):
                g = r1 / r2 - 1
                if np.isfinite(g) and -0.5 < g < 1.0:  # Sanity check
...
class DCFEngine:
    ...
    def auto_tune(self, historical: Dict) -> DCFAssumptions:
        ...
        def _avg(lst, floor=None, cap=None):
            # FIXED: Filter out NaN/None before computing mean
        ...
        # Revenue growth → declining schedule
        ...
        if hist_g is not None:
            ...
            a.mc_revenue_growth_std = std_g if std_g else 0.03
        ...
        if hist_m is not None:
            ...
            a.mc_ebitda_margin_std = std_m if std_m else 0.04
    ...
    def project_fcf(self, growth_rates=None, ebitda_margin=None
        ...
        for i in range(a.projection_years):
            yr = i + 1
            # Handle potential NaN in growth rates
            valid_rates = [r if np.isfinite(r) else 0.05 for r in rates[:yr]]
            ...
            rows.append({
                ...
                "Label": f"FY{2024 + yr}",
                "Revenue": rev,
                "Revenue_Growth": rates[i] if np.isfinite(rates[i]) else 0.05,
                "EBITDA": ebitda,
...
class ScenarioAnalyzer:
    ...
    def _bump_growth(self, base_rates, bump):
        return [max((r if np.isfinite(r) else 0.05) + bump, -0.10)
                for r in base_rates]
    ...
    def run_scenarios(self) -> Dict:
        ...
        bull = self.engine.run_valuation(
            ...
            wacc_override=max(base_wacc + a.bull_wacc_shift, 0.04),
        )
        bull["label"] = "🐂 Bull"
        ...
        bear = self.engine.run_valuation(
            ...
            ebitda_margin=max(a.ebitda_margin + a.bear_margin_cut, 0.10),
...
class MonteCarloSimulator:
    ...
    def _sample(self, mean, std, low, high, size):
        # FIXED: Handle NaN mean/std with sensible defaults
...
class CompsAnalyzer:
    def __init__(self, apple_fin: Dict, peer_data: pd.DataFrame):
        self.fin = apple_fin
    ...
    def peer_table(self) -> pd.DataFrame:
        ...
        combined = pd.concat([apple, df], ignore_index=True)
...
def plot_fcf_projections(results):
    ...
    ax.legend()
    ax.grid(axis="y", alpha=0.3)
    fig.suptitle("Apple — 5-Year Financial Projections", fontweight="bold")
    fig.tight_layout()
    return fig
...
def plot_sensitivity(results, financials):
    ...
    n = len(proj)
    wacc_r = np.arange(max(base_wacc - 0.025, 0.04),
                       base_wacc + 0.035, 0.005)
    growth_r = np.arange(0.010, 0.045, 0.005)
...
def plot_scenarios(scenarios):
    ...
    for b, v in zip(bars, evs):
        ax.text(b.get_x()+b.get_width()/2, v+20, f"${v:,.0f}B",
                ha="center", fontweight="bold")
...
def plot_comps(comps_table, peer_data):
    ...
    if "PE_Ratio" in numeric.columns:
        ...
        colors = ["#e74c3c" if "AAPL" in str(t) else "#3498db"
                  for t in df["Ticker"]]
    ...
    if "EV_EBITDA" in numeric.columns:
        ...
        colors = ["#e74c3c" if "AAPL" in str(t) else "#2ecc71"
                  for t in df["Ticker"]]
...
class UnitTests:
    ...
    def test_terminal_value(cls):
        ...
        engine = cls._make_engine()
        tv = engine.terminal_value(100, 0.10, 0.025)
        ...
        tv_low = engine.terminal_value(100, 0.10, 0.02)
        tv_high = engine.terminal_value(100, 0.10, 0.03)
        ...
        tv_200 = engine.terminal_value(200, 0.10, 0.025)
    ...
    def test_full_valuation(cls):
        ...
        v_low = engine.run_valuation(growth_rates=[0.02]*5)["value_per_share"]
        v_high = engine.run_valuation(growth_rates=[0.12]*5)["value_per_share"]
        cls._assert(v_high > v_low, "Higher growth → higher value")
        v_low_wacc = engine.run_valuation(wacc_override=0.07)["value_per_share"]
        v_high_wacc = engine.run_valuation(wacc_override=0.14)["value_per_share"]
    ...
    def test_auto_tune(cls):
        ...
        hist = {
            "revenue_growth_rates": [0.05, 0.08, 0.07],
            "ebitda_margins": [0.30, 0.32, 0.31],
            "capex_pcts": [0.04, 0.035],
            "depr_pcts": [0.03, 0.028],
            "tax_rates": [0.15, 0.14, 0.16],
        }
        tuned = engine.auto_tune(hist)
        cls._assert(abs(tuned.ebitda_margin - np.mean([0.30, 0.32, 0.31])) < 0.01,
                    "EBITDA margin tuned to historical average")
        cls._assert(abs(tuned.tax_rate - np.mean([0.15, 0.14, 0.16])) < 0.01,
    ...
    def test_monte_carlo(cls):
        ...
        cls._assert(50 < mean_val < 500, f"Mean value reasonable (${mean_val:,.0f})")
    ...
    def test_nan_handling(cls):
        ...
        engine = cls._make_engine()
        hist_with_nan = {
            "revenue_growth_rates": [0.05, np.nan, 0.07, None],
            "ebitda_margins": [0.30, np.nan, 0.31],
            "capex_pcts": [np.nan, np.nan],
            "depr_pcts": [],
            "tax_rates": [0.15, np.nan],
        }
...
def main():
    SEP = "=" * 70
    print(SEP)
    print("APPLE INC. (AAPL) — COMPLETE DCF VALUATION")
    print(SEP)
    ...
    print(f"\n{SEP}")
    print("📦 STEP 1: Collecting Apple's financial data...")
    print(SEP)
